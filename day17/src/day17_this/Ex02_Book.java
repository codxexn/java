...
class Book {
	...
	Book() {
		...
		this("콩쥐팥쥐", 10000);	// 다른 생성자 중에서 "파라미터 2개"를 처리하는 생성자를 호출한다.
		// System.out.println("Book() 생성자 호출");
	}
	
	Book(String title, int price) {
		this(title, "작자미상", price);	// 다른 생성자 중에서 "파라미터 3개" 처리하는 생성자를 호출한다.
		// System.out.println("Book(String title, int price) 생성자 호출");
	}
	...
	Book(String title, String writer, int price) {
		...
		this.writer = writer;
		this.price = price;
		// System.out.println("Book(String title, String writer, int price) 생성자 호출");
	}
